...
export default function PlacesResults({ resultados = lugaresDefault }) {
  ...
  const similitudPromedio = lugares.length > 0
    ? Math.round(
        lugares.filter((l) => typeof l.similitud === "number").reduce((sum, l) => sum + l.similitud, 0) /
          lugares.filter((l) => typeof l.similitud === "number").length
      )
    : 0;
    ...
    <div className="w-full min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-green-50 px-4 md:px-8 py-12">
        ...
        {lugares.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-2xl shadow-md p-5 border-l-4 border-blue-500 hover:shadow-lg transition-all duration-300 transform hover:scale-105">
              <div className="text-3xl font-black text-blue-600">{lugares.length}</div>
              <div className="text-sm text-gray-600 font-semibold">Destinos disponibles</div>
            </div>
      ...
      {lugares.length === 0 ? (
        <div className="max-w-7xl mx-auto text-center py-16 bg-white rounded-xl shadow">
          ...
          <p className="text-gray-500 mt-2">Intenta ajustar tus filtros y buscar de nuevo.</p>
        </div>
      ) : (
        <div className="max-w-7xl mx-auto space-y-12">
          {/* SECCIÓN 1: LUGAR PRINCIPAL DESTACADO */}
          {lugarPrincipal && (
              ...
              <div className="flex gap-8 flex-col xl:flex-row">
                ...
                <div className="flex-1">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {lugaresRecomendados.map((lugar, idx) => (
      ...
      {/* Modal de detalles */}
      {selectedPlace && (
        <DetailModal lugar={selectedPlace} onClose={handleCloseDetails} />
      )}
    </div>
  );
}
