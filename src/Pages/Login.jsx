...
import { getUsers, findUserByEmail, registerUser } from "../service/api.js";
...
const Login = () => {
  ...
  const handleGoogleLogin = () => {
    // Flujo simulado de login con Google (para desarrollo/demo)
    (async () => {
      try {
        const googleEmail = window.prompt(
          "Introduce tu correo de Google para simular el inicio de sesión (ej: usuario@gmail.com):"
        );
        if (!googleEmail) return;

        const found = await findUserByEmail(googleEmail);
        let user;
        if (found && found.length > 0) {
          user = found[0];
        } else {
          // Crear usuario provisional con provider 'google'
        ...
        localStorage.setItem("authToken", user.id || "session");
        navigate("/app", { replace: true });
      } catch (err) {
        console.error(err);
  ...
  const handleFacebookLogin = () => {
    // Flujo simulado de login con Facebook (para desarrollo/demo)
    (async () => {
      try {
        const fbEmail = window.prompt(
          "Introduce tu correo de Facebook para simular el inicio de sesión (ej: usuario@facebook.com):"
        );
        if (!fbEmail) return;

        const found = await findUserByEmail(fbEmail);
        let user;
        if (found && found.length > 0) {
          user = found[0];
        } else {
          // Crear usuario provisional con provider 'facebook'
          user = await registerUser({
